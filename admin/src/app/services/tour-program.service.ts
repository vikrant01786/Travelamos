...
import { Tour_Program } from '../models/tour-program.model';
...
export class TourProgramService {
  ...
  saveAllAdminData(data: any, mode: string): Observable<any> {
    console.log(data, 'Tour Program data from service');
    const headers = new HttpHeaders().set('Content-Type', 'application/json');
    const params = new HttpParams().set('mode', mode);
    return this.http.post<any>(this.url, data, { headers, params}).pipe(catchError(this.handleError));
    }

    getAllTourProgram(mode: string, Lang): Observable<Tour_Program[]> {
      const headers = new HttpHeaders().set('Content-Type', 'application/json');
      const params = new HttpParams().set('mode', mode).set('Lang',Lang);
      return this.http.get<Tour_Program[]>(this.url, { headers, params});
    }


     //delete function
     deleteTourProgram(Id, mode: string): Observable<any> {
      const headers = new HttpHeaders().set('Content-Type', 'application/json');
